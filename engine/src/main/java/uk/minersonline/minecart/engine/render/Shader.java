...
public class Shader implements Destroyable {
	...
	public Shader(List<ShaderModuleData> shaderModuleDataList) {
		...
		shaderModuleDataList.forEach(s -> {
			if (!s.isResource) {
				shaderModules.add(createShader(FileUtils.readFile(s.shaderFile), s.shaderType));
			} else {
				shaderModules.add(createShader(FileUtils.readResource(s.shaderFile), s.shaderType));
			}
		});
	...
	public record ShaderModuleData(String shaderFile, int shaderType, boolean isResource) {
	}
}